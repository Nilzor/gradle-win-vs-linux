...
public class ADF implements ResultProvider {
    ArrayList<ResultProvider> mChildren;
    private static final int CHILD_COUNT = 5;

    public ADF() {
        mChildren = new ArrayList<>(CHILD_COUNT);
        mChildren.add(new ADFB());
        mChildren.add(new ADFC());
        mChildren.add(new ADFD());
        mChildren.add(new ADFE());
        mChildren.add(new ADFF());
