...
public class ADE implements ResultProvider {
    ArrayList<ResultProvider> mChildren;
    private static final int CHILD_COUNT = 5;

    public ADE() {
        mChildren = new ArrayList<>(CHILD_COUNT);
        mChildren.add(new ADEB());
        mChildren.add(new ADEC());
        mChildren.add(new ADED());
        mChildren.add(new ADEE());
        mChildren.add(new ADEF());
