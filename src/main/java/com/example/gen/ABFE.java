...
public class ABFE implements ResultProvider {
    ArrayList<ResultProvider> mChildren;
    private static final int CHILD_COUNT = 5;

    public ABFE() {
        mChildren = new ArrayList<>(CHILD_COUNT);
        mChildren.add(new ABFEB());
        mChildren.add(new ABFEC());
        mChildren.add(new ABFED());
        mChildren.add(new ABFEE());
        mChildren.add(new ABFEF());
