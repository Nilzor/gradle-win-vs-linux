...
public class ACB implements ResultProvider {
    ArrayList<ResultProvider> mChildren;
    private static final int CHILD_COUNT = 5;

    public ACB() {
        mChildren = new ArrayList<>(CHILD_COUNT);
        mChildren.add(new ACBB());
        mChildren.add(new ACBC());
        mChildren.add(new ACBD());
        mChildren.add(new ACBE());
        mChildren.add(new ACBF());
